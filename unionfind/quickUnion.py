...
class QuickUnion(UnionFind):
    ...
    def makeSet(self, e):
        """
        PUo' essere anche >1, nei nostri esempi ha un solo nodo:  (e) senza altre aggiunte.
    ...
    def union(self, rootA, rootB):
        ...
        if rootA == rootB:
            return
        rootB.father = rootA            # Il padre di B è A
        rootA.sons.append(rootB)        # Aggiungo ai figli di A anche B, poichè B figlio di A.
    ...
    def findRoot(self, node):
        ...
        while root.father is not None:  # root non e' ancora la radice
            root = root.father          # Trova il padre del padre del padre.... finche' non arriviam alla radice!
        return root
...
class QuickUnionBalancedPathCompression(QuickUnionBalanced): # tramite euristica path compression, TUTTI i nodi diventano figli della radice.
    """
...
class QuickUnionBalancedPathSplitting(QuickUnionBalanced): # u(i) figlio del nonno u(i+2)
    """
