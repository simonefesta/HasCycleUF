...
class QuickFind(UnionFind):
    ...
    def makeSet(self, e):
        """
        Crea un nuovo albero di altezza 1. (NON PUO' ESSERE MAGGIORE DI 1)
        ...
        """
        root = UFNode(e)                # La radice è un nodo semplice con elemento e
        node = UFNode(e)                # Il nodo è lo stesso nodo semplice e.
        root.sons.append(node)          # Alla radice aggiungo nodo.
        node.father = root              # Definisco il padre come radice.
        self.nodes.append(node)         # Aggiungo il nodo alla lista dei nodi appartenenti all'insieme.
    ...
    def union(self, rootA, rootB):
        ...
        if rootA == rootB:  # si vuole fondere lo stesso albero!
            return          # fai niente
        for sonB in rootB.sons:      # Per ogni figlio della radice di B
            rootA.sons.append(sonB)  # sposta tutti i figli di B in A
            sonB.father = rootA      # rootA diventa il nuovo padre dei figli.
    ...
    def __printFromRoot(self, root):        # Stampa dalla radice di nome "elem" tutti i figli.
